...
TEST(JtKeys, AnEmptyObjectHasNoKeys) { EXPECT_EQ(jt::keys(doc("{}")).to_json(), "[]"); }

TEST(JtKeys, AnArrayHasNoKeys) { EXPECT_THROW(jt::keys(doc("[1,2]")), jt::Error); }

TEST(JtKeys, AScalarHasNoKeys) { EXPECT_THROW(jt::keys(doc(R"({"a":1})"), "/a"), jt::Error); }
...
TEST(JtKeys, RelativePathIsAnError) { EXPECT_THROW(jt::keys(doc(R"({"a":1})"), "a"), jt::Error); }
