...
TEST(JtType, NamesEveryJsonType) {
    const auto sample = doc(R"({"o":{},"a":[],"s":"x","n":1,"b":true,"z":null})");
...
TEST(JtType, ReachesNestedPaths) {
    EXPECT_EQ(jt::type(doc(R"({"a":{"b":[1]}})"), "/a/b/0").to_json(), R"("number")");
}
...
TEST(JtType, RelativePathIsAnError) { EXPECT_THROW(jt::type(doc(R"({"a":1})"), "a"), jt::Error); }
