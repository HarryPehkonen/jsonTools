...
TEST(JtFilter, SupportsEveryOperator) {
  ...
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Eq, doc("2")).to_json(),
            R"([{"a":2}])");
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Ne, doc("2")).to_json(),
            R"([{"a":1},{"a":3}])");
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Gt, doc("2")).to_json(),
            R"([{"a":3}])");
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Ge, doc("2")).to_json(),
            R"([{"a":2},{"a":3}])");
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Lt, doc("2")).to_json(),
            R"([{"a":1}])");
  EXPECT_EQ(jt::filter(list, "/", "a", jt::Op::Le, doc("2")).to_json(),
            R"([{"a":1},{"a":2}])");
}

TEST(JtFilter, KeepsTheOriginalOrder) {
  auto out = jt::filter(doc(R"([{"a":3},{"a":1},{"a":2}])"), "/", "a",
                        jt::Op::Ge, doc("2"));
  EXPECT_EQ(out.to_json(), R"([{"a":3},{"a":2}])");
}

TEST(JtFilter, ReachesNestedKeysWithARelativePath) {
  auto out = jt::filter(doc(R"([{"n":{"last":"a"}},{"n":{"last":"b"}}])"), "/",
                        "n/last", jt::Op::Eq, doc(R"("b")"));
  EXPECT_EQ(out.to_json(), R"([{"n":{"last":"b"}}])");
}

TEST(JtFilter, AnElementMissingTheKeyIsDroppedSilently) {
  auto out = jt::filter(doc(R"([{"a":1},{"b":2}])"), "/", "a", jt::Op::Eq,
                        doc("1"));
...
TEST(JtFilter, EqualityNeverCoercesAcrossTypes) {
  auto out = jt::filter(doc(R"([{"a":1},{"a":"1"}])"), "/", "a", jt::Op::Eq,
                        doc("1"));
  EXPECT_EQ(out.to_json(), R"([{"a":1}])");
}

TEST(JtFilter, OrderingAcrossTypesIsAnError) {
  EXPECT_THROW(jt::filter(doc(R"([{"a":"x"}])"), "/", "a", jt::Op::Gt, doc("1")),
               jt::Error);
}

TEST(JtFilter, OrderingAgainstANonScalarLiteralIsAnError) {
  EXPECT_THROW(jt::filter(doc(R"([{"a":1}])"), "/", "a", jt::Op::Gt, doc("[]")),
               jt::Error);
}
...
TEST(JtFilter, AKeyWithALeadingSlashIsAnError) {
  EXPECT_THROW(jt::filter(doc(R"([{"a":1}])"), "/", "/a", jt::Op::Eq, doc("1")),
               jt::Error);
}
...
TEST(JtFilter, MissingListPathIsAnError) {
  EXPECT_THROW(jt::filter(doc(R"({"items":[]})"), "/nope", "a", jt::Op::Eq,
                          doc("1")),
               jt::Error);
}

TEST(JtFilter, OutputIsNeverLongerThanInput) {
  auto out = jt::filter(doc(R"([{"a":1},{"a":2},{"a":3}])"), "/", "a",
                        jt::Op::Ge, doc("0"));
