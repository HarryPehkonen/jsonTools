...
public:
    Error(std::string path, std::string problem, std::string suggestion = "")
        : std::runtime_error(format(path, problem, suggestion)), path_(std::move(path)),
          problem_(std::move(problem)), suggestion_(std::move(suggestion)) {}
...
// enough.
std::string nearest_key(const std::vector<std::string>& candidates, const std::string& target);
...
template <typename Body> int run_cli(Body&& body) {
    try {
