...
bool first_too_deep(const jsom::JsonDocument& node, const std::string& pointer, int depth,
                    int limit, std::string& hit) {
    ...
    if (node.is_object()) {
        for (const auto& entry : node.items()) {
            std::string child = pointer + "/" + jsom::JsonPointer::escape_segment(entry.first);
            if (first_too_deep(entry.second, child, depth + 1, limit, hit))
                return true;
        }
    } else {
        ...
        for (std::size_t i = 0; i < arr.size(); ++i) {
            ...
            if (first_too_deep(arr[i], child, depth + 1, limit, hit))
                return true;
        }
    }
    return false;
...
private:
    ...
    void object(const std::string& pointer) {
        ...
        while (pos_ < text_.size()) {
            ...
            std::string child = pointer + "/" + jsom::JsonPointer::escape_segment(key);
...
FromResult from_text(const std::string& text, const std::string& origin, const FromOptions& opts) {
    if (opts.max_size >= 0 && static_cast<long long>(text.size()) > opts.max_size) {
        throw Error(origin,
