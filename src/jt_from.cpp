...
namespace {
const char* kUsage =
    "jtFrom <file> [--check-only] [--max-depth N] [--max-size N] "
...
int main(int argc, char* argv[]) {
  ...
  if (!have_file) jt::fail("<args>", "missing <file> argument", kUsage);
  ...
  return jt::run_cli([&] {
    ...
    if (!opts.check_only) jt::write_stdout(result.doc, globals.pretty);
  });
}
