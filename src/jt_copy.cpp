...
namespace {
const char* kUsage = "jtCopy <from> <to> [--if-not-set | --replace] [--pretty]";
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (jt::take_global(arg, "jtCopy", kUsage, globals)) {
            if (globals.exit_now)
                return 0;
        } else if (arg == "--if-not-set" || arg == "--replace") {
            if (mode_set)
                jt::fail("<args>", "--if-not-set and --replace are exclusive", kUsage);
    ...
    if (positional.size() != 2)
        jt::fail("<args>", "expected <from> and <to>", kUsage);

    return jt::run_cli([&] {
        jsom::JsonDocument doc = jt::read_stdin();
        doc = jt::copy(std::move(doc), positional[0], positional[1], mode);
