...
const jsom::JsonDocument& require_array(const jsom::JsonDocument& doc, const std::string& pointer,
                                        const char* role) {
...
jsom::JsonDocument zip(jsom::JsonDocument doc, const std::string& keys_path,
    ...
    if (keys.size() != values.size()) {
        throw Error(display_pointer(keys_pointer),
                    "the key and value lists differ in length (" + std::to_string(keys.size())
    ...
    for (std::size_t i = 0; i < keys.size(); ++i) {
        ...
        if (!overwrite && zipped.contains(key)) {
            throw Error(keys_pointer + "/" + std::to_string(i), "duplicate key '" + key + "'",
