...
jsom::JsonDocument values_array(const jsom::JsonDocument& doc, const std::string& path) {
    ...
    if (!target.is_object()) {
        throw Error(display_pointer(pointer), "a " + type_name(target) + " has no values",
...
jsom::JsonDocument values(jsom::JsonDocument doc, const std::string& obj_path,
    ...
    jsom::JsonDocument arr = values_array(jsom::JsonDocument(doc), normalize_pointer(obj_path));
