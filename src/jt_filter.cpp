...
namespace {
const char* kUsage = "jtFilter [<listPath>] <keyPath> "
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (jt::take_global(arg, "jtFilter", kUsage, globals)) {
            ...
            if (have_op)
                jt::fail("<args>", "only one comparison is allowed", kUsage);
            op = parsed;
    ...
    if (!have_key)
        jt::fail("<args>", "missing <keyPath>", kUsage);
    if (!have_op)
        jt::fail("<args>", "missing a comparison flag", kUsage);
