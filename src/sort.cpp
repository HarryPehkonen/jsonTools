...
void require_comparable(const jsom::JsonDocument& value, const jsom::JsonDocument* first,
                        const std::string& at, const std::string& where) {
    if (!is_orderable(value)) {
        throw Error(
            at, "cannot sort " + where + ": " + type_name(value) + " values have no natural order",
            "sort by a scalar key instead, e.g. jtSort /items name");
    }
    if (first != nullptr && first->type() != value.type()) {
        throw Error(
            at, "mixed types in " + where + ": " + type_name(*first) + " and " + type_name(value),
...
std::vector<std::vector<const jsom::JsonDocument*>>
collect_keys(const std::vector<jsom::JsonDocument>& elements, const std::vector<SortKey>& keys,
             const std::string& list_pointer) {
    ...
    for (const SortKey& key : keys) {
        ...
        for (std::size_t i = 0; i < elements.size(); ++i) {
            ...
            if (!elements[i].is_object()) {
                throw Error(at, "a sort key needs object elements, got a " + type_name(elements[i]),
            ...
            if (value == nullptr) {
                ...
                if (hint.empty())
                    hint = "every element must carry the sort key";
                throw Error(at + key_pointer, "sort key not found", hint);
            }
            require_comparable(*value, column.empty() ? nullptr : column.front(), at + key_pointer,
                               "'" + key.key + "'");
...
jsom::JsonDocument sort(jsom::JsonDocument doc, const std::string& list_path,
    ...
    if (keys.empty()) {
        ...
        std::stable_sort(
            ...
            [](const jsom::JsonDocument& a, const jsom::JsonDocument& b) { return a < b; });
    } else {
        ...
        for (std::size_t index : order)
            sorted.push_back(elements[index]);
