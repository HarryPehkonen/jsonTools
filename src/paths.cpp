...
std::string display_pointer(const std::string& pointer) { return pointer.empty() ? "/" : pointer; }
    ...
    for (const std::string& segment : segments) {
        ...
        if (next == nullptr) {
            if (current->is_object()) {
                ...
                if (!best.empty()) {
                    return "did you mean " + prefix + "/" + jsom::JsonPointer::escape_segment(best)
                           + "?";
                }
            }
            return "";
...
// the range.
static std::string array_bounds_hint(const std::string& pointer, std::size_t size,
                                     bool with_indexes) {
    ...
    if (with_indexes)
        hint += " (indexes 0-" + std::to_string(size - 1) + ")";
    return hint;
}
...
const jsom::JsonDocument& require_at(const jsom::JsonDocument& doc, const std::string& pointer) {
    ...
    const jsom::JsonDocument* current = &doc;
    for (const std::string& segment : segments) {
        const std::string child = prefix + "/" + jsom::JsonPointer::escape_segment(segment);
        if (current->is_object()) {
            if (!current->contains(segment)) {
                // A genuinely absent key — the one case that is truly "not found";
                // it keeps the typo hint.
                throw Error(display_pointer(child), "path not found", suggest_for(doc, pointer));
            }
            ...
            if (!jsom::JsonPointer::is_array_index(segment)) {
                throw Error(display_pointer(child), "'" + segment + "' is not an array index",
                            "array elements are addressed by index, e.g. " + display_pointer(prefix)
                                + "/0");
            }
            ...
            if (index >= size) {
                throw Error(display_pointer(child), "index " + segment + " is out of range",
            ...
            throw Error(display_pointer(child),
                        "cannot look up '" + segment + "' inside a " + type_name(*current),
                        "use " + display_pointer(prefix) + " to address the " + type_name(*current)
                            + " itself");
        }
        prefix = child;
    }
...
void require_parent(const jsom::JsonDocument& doc, const std::string& pointer,
    ...
    if (container->is_object())
        return; // any leaf key can live in an object

    if (container->is_array()) {
        const std::string leaf = jsom::JsonPointer::get_last_segment(pointer);
        if (jsom::JsonPointer::is_append(leaf))
            return; // "-" grows the array by one
        if (!jsom::JsonPointer::is_array_index(leaf)) {
            throw Error(display_pointer(pointer), "'" + leaf + "' is not an array index",
                        "array elements are addressed by index, e.g. " + display_pointer(parent)
                            + "/0");
        }
        // to_array_index overflows to an exception for indexes beyond size_t;
        // such an index is certainly out of range, so keep the default.
        ...
        if (size > 0 && index == size) {
            ...
            hint
                = array_bounds_hint(parent, size, false) + "; append with the '-' sentinel instead";
        } else {
            hint = array_bounds_hint(parent, size, true);
        }
        throw Error(display_pointer(pointer), "index " + leaf + " is out of range", hint);
    }

    throw Error(display_pointer(parent), "cannot put a value inside a " + type_name(*container),
...
void create_object_path(jsom::JsonDocument& doc, const std::string& pointer) {
    ...
    for (std::size_t i = 0; i + 1 < segments.size(); ++i) {
        ...
        if (current->is_array()) {
            ...
            if (!jsom::JsonPointer::is_array_index(segment)) {
                throw Error(display_pointer(here), "'" + segment + "' is not an array index",
                            "array elements are addressed by index, e.g. " + display_pointer(prefix)
                                + "/0");
            }
            // to_array_index overflows to an exception for indexes beyond size_t;
            ...
            if (index >= size) {
                ...
                throw Error(display_pointer(here), "index " + segment + " is out of range", hint);
            }
            current = &(*current)[index];
