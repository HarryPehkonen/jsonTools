...
| `jtSort` | Sort a list (`jtSort <listPath> <key1> --desc-for <key2> <key3>`). Multiple keys, mixed directions. Bare `jtSort` (no arguments at all) is settled behavior: it natural-sorts the whole document (`jtSort /` with no keys). Keys must be non-empty relative paths, validated before the list is resolved (like `jtFilter`, review issue 5). No keys → sort by element natural type (numbers numerically, strings lexically); **mixed types = error**. "Typical JavaScript" coercion is a **future switchable option**. |
